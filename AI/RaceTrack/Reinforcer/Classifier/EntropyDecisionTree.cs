...
namespace Classifier
{
    ...
    /// </summary>
    public class EntropyDecisionTree : DecisionTree
    {
            ...
            List<ClassifierAttribute> attributes, bool prune)
        {
            tree = DTL(examples, attributes, 
                DataLoader.LoadPossibleClassifications(examples), 
                MajorityValue(examples), prune);
        }

        public EntropyDecisionTree(List<ExampleInstance> examples, bool prune)
        {
            tree = DTL(examples, DataLoader.GenerateAttributes(examples),
                DataLoader.LoadPossibleClassifications(examples), MajorityValue(examples), prune);
        }
        ...
        static ClassifierAttribute SelectBestAttribute(List<ClassifierAttribute> attributes,
            List<ExampleInstance> examples, List<string> posClasses)
        {
            ...
            {
                double g = ComputeGain(examples, ca, posClasses.ToArray());
                                      ...
                                      string def, bool prune)
        {
            ...
            ClassifierAttribute bestAttribute = SelectBestAttribute(attributes, examples, posClass);
            ...
            foreach (string attributeValue in bestAttribute.Values)
            {
                ...
                TreeNode<Decision> tn = DTL(examplesSubset, tempAttributes, posClass, majorityValue,prune);
	...
	16.92 , 18.31 , 19.68, 21.03, 22.36, 23.69, 25.00, 26.30, 27.59, 
                                             28.87, 30.14,31.41, 32.67};
        ...
        private static double ComputeGain(List<ExampleInstance> examples,
            ClassifierAttribute attribute, string [] classifications)
        {
            ...
            {
                aList.Add(CountPositives(examples, classifications[i])); 
            }
            double totalEntropy = CalculateEntropy(aList); 
            ...
            for (int i = 0; i < values.Count; i++)
            {
                ...
                {
                    CountClassifcations(examples, attribute, values[i],
                        classifications[j], out positives, out negatives);
                    aList.Add(positives); 
                }
                double entropy = CalculateEntropy(aList);
        ...
        private static double CalculateEntropy(List<int> counts) //int positives, int negatives)
        {
