 ...
 * @author juwei
 */
public class ViewLabeledSetWithLabelDegree extends Transformer {
	/**
	 * Default constructor.
	 */
	public ViewLabeledSetWithLabelDegree() {
		super();
	}
	 ...
	 * @param dest
	 */
	public ViewLabeledSetWithLabelDegree(Path src, Path dest) {
		super(src, dest);
	}
	...
	public static class MapClass extends MapReduceBase implements
			Mapper<Text, LabeledAdjSetVertexWithTwoHopLabel, Text, Text> {
		@Override
		public void map(Text key, LabeledAdjSetVertexWithTwoHopLabel value,
				OutputCollector<Text, Text> output, Reporter reporter)
				throws IOException {
			output.collect(new Text(value.getNeighbors().size() + ""), key);
		}
	}
	...
	public void execute() throws ProcessorExecutionException {
		JobConf jobConf = new JobConf(conf,
				ViewLabeledSetWithLabelDegree.class);
	...
	public static void main(String[] args) {
		try {
			int res = ToolRunner.run(new ViewLabeledSetWithLabelDegree(),args);
			System.exit(res);
		} catch (Exception e) {
			e.printStackTrace();
		}
