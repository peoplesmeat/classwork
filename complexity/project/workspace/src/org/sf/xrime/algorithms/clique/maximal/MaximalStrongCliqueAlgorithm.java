...
/**
 * This algorithm is used to calculate all maximal strong cliques in a graph (not 
 * necessarily connected). The input is in the form of outgoing adjacency lists.
 * @author xue
 *
 */
public class MaximalStrongCliqueAlgorithm extends GraphAlgorithm {
  /**
   * Default constructor.
   */
  public MaximalStrongCliqueAlgorithm(){
    super();
  }
  ...
  public void execute() throws ProcessorExecutionException {
    try {
      ...
      String temp_dir_prefix = getDestination().getPath().getParent().toString()+"/msc_"+
      ...
      System.out.println("++++++>"+dirMgr.getSeqNum()+": StrongNeighborhoodGenerate");
      src = new Graph(Graph.defaultGraph()); 
      src.setPath(getSource().getPath());
      dest = new Graph(Graph.defaultGraph());
      tmpDir = dirMgr.getTempDir();
      dest.setPath(tmpDir);
      GraphAlgorithm gen_neighbor = new StrongNeighborhoodGenerate();
  ...
  public static void main(String[] args){
    try {
      int res = ToolRunner.run(new MaximalStrongCliqueAlgorithm(), args);
      System.exit(res);
    } catch (Exception e) {
      e.printStackTrace();
    }
