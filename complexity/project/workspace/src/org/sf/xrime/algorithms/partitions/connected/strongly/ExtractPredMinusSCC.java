...
/**
 * This class is used to extract the predecessors minus the SCC.
 * @author xue
 */
public class ExtractPredMinusSCC extends GraphAlgorithm {
  /**
   * Default constructor.
   */
  public ExtractPredMinusSCC(){
    super();
  }
  /**
   * Mapper.
  ...
  public static class MapClass extends GraphAlgorithmMapReduceBase implements
    ...
    public void map(Text key, LabeledAdjBiSetVertex value,
        ...
        throws IOException {
      // Pred(G,v)\SCC(G,v) = vertexes which have backward label but do not have
      // forward label.
      if(value.getStringLabel(ConstantLabels.BACKWARD_LABEL)!=null &&
         value.getStringLabel(ConstantLabels.FORWARD_LABEL)==null){
        // Notify adjacency vertexes about what lines should be retained.
        ...
        value.clearLabels();
        // The bi-set could be cleared too. We could make them up in reducer.
  ...
  public void execute() throws ProcessorExecutionException {
    JobConf conf = new JobConf(context, ExtractPredMinusSCC.class);
    conf.setJobName("ExtractPredMinusSCC");
