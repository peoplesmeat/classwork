 ...
 * the License.
 */
package org.sf.xrime.algorithms.layout.circular;
...
public class CoordinatesCalculation extends GraphAlgorithm {
   ...
   * The number assignment is sequantial, but this calculation could be parallel.
   * @author xue
   */
  public static class MapClass extends GraphAlgorithmMapReduceBase implements
    ...
    public void map(Text key, LabeledAdjSetVertex value,
      ...
      int radius = (max_x > max_y ? max_y/2 : max_x/2) - (max_x > max_y ? max_y/2 : max_x/2)/10;
      
      // Calcualte coordinates.
      int seq_num = ((IntWritable)value.getLabel(ConstantLabels.SEQUENTIAL_NUM)).get();
      double x_coordinate = origin_x + radius * Math.cos(seq_num * ((2*Math.PI)/num_of_vertexes));
      double y_coordinate = origin_y - radius * Math.sin(seq_num * ((2*Math.PI)/num_of_vertexes));
      
      LabeledAdjSetVertex result = new LabeledAdjSetVertex((AdjSetVertex)value);
      
      result.setLabel(ConstantLabels.X_COORDINATE, new DoubleWritable(x_coordinate));
      result.setLabel(ConstantLabels.Y_COORDINATE, new DoubleWritable(y_coordinate));
