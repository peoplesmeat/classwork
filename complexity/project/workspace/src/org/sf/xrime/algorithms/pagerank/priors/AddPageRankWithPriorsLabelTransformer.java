 ...
 * @author Cai Bin
 */
public class AddPageRankWithPriorsLabelTransformer extends Vertex2LabeledTransformer {
  static final public String pageRankWithPriorsInitValueKey = "xrime.algorithm.pageRankWithPriors.initValue";
   ...
   * Default constructor.
   */
  public AddPageRankWithPriorsLabelTransformer(){
    super();
  }
   ...
   */
  public AddPageRankWithPriorsLabelTransformer(Path src, Path dest) {
    super(src, dest);
  }
  ...
  public void execute() throws ProcessorExecutionException {
    setLabelAdderClass(PageRankWithPriorsLabelAdder.class);
    setOutputValueClass(LabeledAdjSetVertex.class);
    conf.set(pageRankWithPriorsInitValueKey, Double.toString(initValue));
    super.execute();
  }
   ...
   * @author Cai Bin
   */
  public static class PageRankWithPriorsLabelAdder extends LabelAdder {
    private double initValue=0;
    ...
    public void addLabels(Labelable labels, Element element) {
      ...
      SetBFSLabel setBFSLabel=(SetBFSLabel) labels.getLabel(SetBFSLabel.setBFSLabelPathsKey);
            
      if(label==null) {
        label=new PageRankLabel(0);
      }
      
      if(setBFSLabel==null || setBFSLabel.getStatus()==0) {
        ...
        if(setBFSLabel.getDistance()>0) {
          ...
          label.setPr(initValue);
          label.setInitWeight(initValue);
          label.setInitVertex(true);
        }        
      }
      ...
      {
        super.configure(job);
        String property=job.get(pageRankWithPriorsInitValueKey);
        if(property!=null) {
  ...
  public static void main(String[] args) {
    try {
      int res = ToolRunner.run(new AddPageRankWithPriorsLabelTransformer(), args);
      System.exit(res);
    } catch (Exception e) {
      e.printStackTrace();
    }
