...
/**
 * Transform outgoing adjacent vertexes lists into the data structure suitable
 * for some algorithms. The output AdjBiSetVertex collects the opposites of
 * incoming and outgoing incident edges into 2 sets.
 * 
 * @author xue
 */
public class OutAdjVertex2AdjBiSetVertexTransformer extends Transformer {
	/**
	 * Default constructor.
	 */
	public OutAdjVertex2AdjBiSetVertexTransformer() {
		super();
	}
	 ...
	 * @param dest
	 */
	public OutAdjVertex2AdjBiSetVertexTransformer(Path src, Path dest) {
		super(src, dest);
	}

	/**
	 * Map outgoing adjacent vertexes lists into AdjBiSetVertex.
	 * 
	 * @author xue
	 */
	public static class MapClass extends MapReduceBase implements
		...
		public void map(Text key, AdjVertex value,
				...
				throws IOException {
			// The vertex of this outgoing adjacent vertexes list.
			...
			for (Edge edge : value.getEdges()) {
				String to_vertex = edge.getTo();
				// Add the other end of the outgoing edge to forward vertex set
				// of the vertex.
				the_vertex.addForwardVertex(new AdjVertexEdge(to_vertex));
				// Create new object for the other end of the outgoing edge.
				AdjBiSetVertex new_vertex = new AdjBiSetVertex();
				new_vertex.setId(to_vertex);
				new_vertex.addBackwardVertex(new AdjVertexEdge(the_id));
				// Collect this one.
				output.collect(new Text(to_vertex), new_vertex);
			}
			// Collect the vertex.
	...
	public void execute() throws ProcessorExecutionException {
		JobConf jobConf = new JobConf(conf,
				OutAdjVertex2AdjBiSetVertexTransformer.class);
	...
	public static void main(String[] args) {
	  try {
      int result = ToolRunner.run(new OutAdjVertex2AdjBiSetVertexTransformer(), args);
      System.exit(result);
    } catch (Exception e) {
      e.printStackTrace();
    }
