...
/**
 * This class is used to calculate k-core with specified k. It wraps the
 * recursive invocation of ElementRemoval. We assume the input is in the form of
 * outgoing adjacency vertexes lists.
 * 
 * @author xue
 */
public class SpecifiedKCoreAlgorithm extends GraphAlgorithm {
  /**
   * Default constructor.
   */
  public SpecifiedKCoreAlgorithm() {
    super();
  }
  ...
  public void execute() throws ProcessorExecutionException {
    try {
      ...
      SequenceTempDirMgr dirMgr = new SequenceTempDirMgr(temp_dir_prefix,
          context);
      ...
      // AdjSetVertex.
      System.out.println("-------->" + dirMgr.getSeqNum()
          + ": Transform input to AdjSetVertex");
      ...
      // 2. Recursively invoke ElementRemoval to remove vertexes, and lines
      // incident with
      ...
      while (true) {
        ...
        System.out.println("-------->" + dirMgr.getSeqNum()
            + ": Recursively delete vertexes and lines");
        GraphAlgorithm element_rm = new ElementRemoval();
        // Inherit settings from this algorithm.
        element_rm.setConf(context);
        src = new Graph(Graph.defaultGraph());
        // Use the output directory of last step as the input directory of this
        // step.
        src.setPath(tmpDir);
        ...
        element_rm.setParameter(ElementRemoval.K_OF_CORE, context
            .getParameter(ElementRemoval.K_OF_CORE));
        element_rm.execute();

        // 2.2. Check for convergence.
        RunningJob conv_result = element_rm.getFinalStatus();
        long found_vertexes_num = MRConsoleReader
            .getReduceOutputRecordNum(conv_result);
  ...
  public static void main(String[] args) {
    try {
      int res = ToolRunner.run(new SpecifiedKCoreAlgorithm(), args);
      System.exit(res);
    } catch (Exception e) {
      e.printStackTrace();
    }
