...
/**
 * Transformer to add BFS label to LabeledAdjVertex
 * @author Cai Bin
 */
public class BFSLabelTransformer extends Vertex2LabeledTransformer {  
  @Override
  ...
  public void execute() throws ProcessorExecutionException {
    setLabelAdderClass(BFSLabelAdder.class);
    setOutputValueClass(LabeledAdjVertex.class);
    super.execute();
  }
   ...
   * @author Cai Bin
   */
  static public class BFSLabelAdder extends LabelAdder {
    /**
     * Add the label.
    ...
    public void addLabels(Labelable labels, Element element) {
      BFSLabel label=(BFSLabel) labels.getLabel(BFSLabel.bfsLabelPathsKey);
      
      if(label==null) {
        label=new BFSLabel();
        labels.setLabel(BFSLabel.bfsLabelPathsKey, label);
      }
    }
  }
  ...
  public static void main(String[] args) {
    try {
      int res = ToolRunner.run(new BFSLabelTransformer(), args);
      System.exit(res);
    } catch (Exception e) {
      e.printStackTrace();
    }
